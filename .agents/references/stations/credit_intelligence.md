...
## 7. Hallazgos de Régimen — Pendiente y Easing (17-Ago-2026)

### Período natural y predictibilidad
- **Half-life (autocorrelación)**: 67.2 pivotes ≈ **269 días** — la más persistente de CAT1.
- **Pendiente en ventana natural vs SPY forward**: Spearman **−0.488** CI95[−0.54,−0.43] — **PREDICTIVA FUERTE**.
- **Dirección**: pendiente CREDIT positiva (spread subiendo, estrés) → SPY BAJA. Señal de lead-indicator de reversión.
- **CONTEMPORÁNEA vs PREDICTIVA**: ρ contemporánea +0.447 (CREDIT sube con SPY), ρ predictiva −0.432 (CREDIT predice reversión). Firma clásica de mean-reversion.

### Asimetría direccional
- **ASIMÉTRICA**: la pendiente negativa (alivio crediticio) predice alza de SPY **+23.0%** (83.8% wins) — 1.62× más fuerte que la pendiente positiva (estrés) predice baja (−14.2%, 71.9% losses).
- El "comprar miedo" crediticio es más rentable que el "vender estrés".
- La asimetría **desaparece en ventana corta** (1 pivote, 4d: ratio 0.96×, simétrica) → es un fenómeno de ventana larga.

### CREDIT easing y pisos de mercado
- **Hipótesis testeada**: "sin crédito la caída no tiene fondo".
- **Veredicto**: REFUTADA como condición necesaria. 75.7% de los pisos reales se formaron SIN easing crediticio.
- **Reformulada**: "sin crédito, la caída es más severa y la recuperación más débil". **CONFIRMADO.**
- **K=1 (~4-5 días antes del piso)**: EASING → forward SPY **+5.19%** (93.75% WR) vs SIN_EASING → +2.99% (75.65% WR). Δ +2.20pp CI95[1.18,3.21].
- **K=3 y K=5**: la señal se diluye completamente (CI95 cruza cero).
- **Severidad**: SIN_EASING tiene 1.7× más drawdowns >10% (18.3% vs 10.7%).
- **Conclusión**: CREDIT ayuda, no manda. El mercado encuentra pisos solo, pero cuando CREDIT confirma con easing, el piso es casi seguro (93.75% WR) y la recuperación es 1.7× más fuerte.

### Limitación de datos
- **Marzo 2020**: spike visible en data diaria (credit_vel = −0.055 el 23-Mar-2020).
- **Agosto 2015 (ETF flash crash)**: el crash fue intraday (APs abandonaron arbitraje, ETFs a descuentos >30% del NAV). La data diaria usó cierres recuperados → el spike está **borrado** de la data oficial. Documentado académicamente (SEC 2015, Dannhauser & Hoseinzade 2021).
- **Implicación**: nuestras mediciones son conservadoras (subestiman el verdadero estrés crediticio en eventos de cola).
