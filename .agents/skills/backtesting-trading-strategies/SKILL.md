...
1. Fetch historical data (cached to `data/` within this skill directory for reuse):
   ```bash
   python .agents/skills/backtesting-trading-strategies/scripts/fetch_data.py --symbol BTC-USD --period 2y --interval 1d
   ```
2. Run a backtest with default or custom parameters:
   ```bash
   python .agents/skills/backtesting-trading-strategies/scripts/backtest.py --strategy sma_crossover --symbol BTC-USD --period 1y
   python .agents/skills/backtesting-trading-strategies/scripts/backtest.py \
   ...
   ```
3. Analyze results saved to `reports/` within this skill directory -- includes `*_summary.txt` (performance metrics), `*_trades.csv` (trade log), `*_equity.csv` (equity curve data), and `*_chart.png` (visual equity curve).
4. Optimize parameters via grid search to find the best combination:
   ```bash
   python .agents/skills/backtesting-trading-strategies/scripts/optimize.py \
...
Create `config/settings.yaml` within this skill directory:
...
See `references/errors.md` within this skill directory for common issues and solutions.

## Examples

See `references/examples.md` within this skill directory for detailed usage examples including:
