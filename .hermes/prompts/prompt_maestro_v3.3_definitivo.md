# Evaluación de la Auditoría Gemini + Prompt Maestro V3.3 Definitivo

**Versión:** 3.3 — 29-Ago-2026 19:40 UTC  
**Cambios vs V3.2:** Documentación del Bayesian Shrinkage en p_bull y ev_net. Documentación del artefacto e_ret_min=-0.02% (frontera muestral). Bridge IMPULSO→TIDE formalizado. Body_when_green/red como tarea pendiente en C.6.  
**Cambios V3.2 vs V3.0:** Lectura DINÁMICA multi-escala de los fact stores. Descomposición EV completa (p_bull × e_ret_max + p_bear × e_ret_min) y R:R por señal y escala.
...
# PARTE II: PROMPT MAESTRO V3.3 DEFINITIVO
...
> [!WARNING]
> **Tabla PROVISIONAL — Pendiente de recomputación bajo Protocolo C.2.** Estos datos usan `WR 20d` porque son los únicos disponibles para canales solitarios. La tabla se preserva como referencia comparativa pero NO constituye evidencia causal (restricción E.9). La recomputación con lectura multi-escala del fact store es obligatoria.
...
> [!CAUTION]
> **Bayesian Shrinkage activo.** Todos los `p_bull` y `ev_net` en los fact stores son POST-SHRINKAGE via `bayesian_shrink_p()` y `bayesian_shrink_ev()` del motor generador. El shrinkage tira p_bull hacia el prior poblacional (conservador). Ejemplo verificado: VIX spike zz75 tiene p_bull_raw=10/16=0.625 pero p_bull_stored=0.577 (-4.8pp). Los EV_total reportados abajo usan p_bull shrunk — son estimaciones CONSERVADORAS. Los EV crudos serían ~1-2pp superiores.
>
> **Artefacto de frontera muestral:** Cuando `n_neg=0` (cero piernas bajistas observadas para un state_key/escala), el motor asigna `e_ret_min = prior[-0.02%]` vía Laplace Shrinkage. El R:R resultante (>500:1 o "∞") NO indica ausencia de riesgo — indica insuficiente evidencia muestral del downside. Tratar estos estados con el protocolo §3.3 de muestras pequeñas.
>
> **e_ret_max y e_ret_min son retornos de PIERNAS ZIGZAG**, no de trades individuales. Una pierna zz75 (≥7.5%) es un movimiento estructural de 20-200+ días con drawdowns internos. No confundir con targets de trades intradía o swing.
...
|---|:-:|---|:-:|:-:|:-:|:-:|:-:|
| FAST_CRUSH (cap. acelerada) | N=15 | zz25 | 0.600 | +6.15% | -7.30% | **+0.77%** | 0.84:1 |
|  |  | zz50 | 0.600 | +9.33% | -6.71% | **+2.92%** | **1.39:1** |
|  |  | zz75 | **0.615** | +11.68% | -0.02%⁽¹⁾ | **+7.18%** | **⁽¹⁾** |
| DECEL_DOWN (cediendo) | N=56 | zz25 | 0.612 | +5.67% | -6.82% | **+0.83%** | 0.83:1 |
|  |  | zz50 | **0.667** | +10.93% | -11.04% | **+3.61%** | 0.99:1 |
|  |  | zz75 | 0.588 | +15.20% | -10.48% | **+4.63%** | **1.45:1** |

→ **BSI washed_out + FAST_CRUSH en zz75:** n_pos=3, n_neg=0. El R:R alto es un **artefacto de frontera muestral** (ver nota ⁽¹⁾ arriba). Con solo 3 piernas alcistas y 0 bajistas matcheadas, la evidencia del downside es insuficiente. **Señal prometedora pero con protocolo §3.3 obligatorio.**  
→ **BSI washed_out + DECEL_DOWN:** N=56 con n_pos/n_neg mejor distribuidos. R:R=1.45:1 en zz75 es robusto. EV=+4.63% post-shrinkage. **Señal TIDE confiable.**
...
DIAGNÓSTICO INTEGRADO:
  ...
  BRIDGE IMPULSO → TIDE (Monitor días +1 a +3):
    Día +1 a +3: ¿Credit.D2 transicionó a DECEL_DOWN?
    SI CONVERGE TIDE:
      → Cancelar TP táctico de zz25
      → Escalar posición a SWING ESTRUCTURAL (barriers zz50)
      → Target ampliado: e_ret_max(zz50) del Credit = +11.19%, e_days=13d
    SI NO CONVERGE (Credit.D2 permanece en STABLE/ACCEL_UP):
      → Ejecutar TP táctico en bar[+1] / zz25
      → Salida limpia, no atraparse en continuación bajista
```
...
- **Body_when_green:** promedio de `(close-open)/open` solo cuando `close > open`
- **Body_when_red:** promedio de `(close-open)/open` solo cuando `close < open`
- **EV_bar1:** `WR_green × body_when_green + WR_red × body_when_red`

> [!NOTE]
> La descomposición body_when_green / body_when_red es el equivalente de bar[+1] a la descomposición EV = p_bull × e_ret_max + p_bear × e_ret_min del fact store. Permite detectar diamantes con WR alto pero EV negativo (si body_when_red >> body_when_green).
...
12. **Consciente del Bayesian Shrinkage.** Los p_bull y ev_net almacenados en los fact stores son POST-SHRINKAGE (conservadores). Al reportar datos, especificar si son shrunk (stored) o raw (n_pos/n_tot). Los e_ret_max y e_ret_min son retornos de PIERNAS ZIGZAG (movimientos de ≥2.5/5/7.5%), NO targets de trades individuales. Cuando n_neg=0, e_ret_min=-0.02% es un artefacto del prior — NO evidencia de "riesgo cero".
