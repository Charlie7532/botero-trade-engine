...
========================================
Vault provider for VVIX Market SIGMET & 3-Day Kinematic Velocity Telemetry.
Reads VVIX from Vault, generates authoritative MarketSIGMET, and persists:
...
class VVIXProvider:
    """Vault provider for CBOE VVIX Vol-of-Vol Index SIGMET and state transitions."""

    name = "vvix_sigmet"
    categories = ["vvix", "volatility"]

    def run_full(self, store: TimescaleDataStore, **kwargs) -> Dict[str, Any]:
        """Compute and persist VVIX Market SIGMET and regime state from Vault data."""
        if _already_vaulted_today(store, "vvix/sigmet", "MARKET"):
            logger.info("📊 VVIX Market SIGMET already vaulted today — skipping")
            return {"status": "skipped", "reason": "already_today"}

        return self._compute(store)

    def run_ticker(self, store: TimescaleDataStore, ticker: str) -> Dict[str, Any]:
        """VVIX SIGMET is market-wide — falls back to run_full."""
        return self._compute(store)
    ...
    def _compute(self, store: TimescaleDataStore) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"📊 VVIX SIGMET Vaulted: State={sigmet.state_key} | "
