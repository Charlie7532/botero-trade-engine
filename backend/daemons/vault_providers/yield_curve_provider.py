...
=====================================================
Vault provider for Yield Curve Market METAR & 3-Day Kinematic Velocity Telemetry.
Reads TNX and IRX from Vault, generates authoritative MarketMETAR, and persists:
...
class YieldCurveProvider:
    """Vault provider for Macro Yield Curve Spread (TNX - IRX) METAR and state transitions."""

    name = "yield_curve"
    categories = ["yield_curve", "macro_yields"]

    def run_full(self, store: TimescaleDataStore, **kwargs) -> Dict[str, Any]:
        """Compute and persist Yield Curve Market METAR and regime state from Vault data."""
        if _already_vaulted_today(store, "yield_curve/sigmet", "MARKET"):
            logger.info("📊 Yield Curve Market METAR already vaulted today — skipping")
            return {"status": "skipped", "reason": "already_today"}

        return self._compute(store)

    def run_ticker(self, store: TimescaleDataStore, ticker: str) -> Dict[str, Any]:
        """Yield Curve METAR is market-wide — falls back to run_full."""
        return self._compute(store)
    ...
    def _compute(self, store: TimescaleDataStore) -> Dict[str, Any]:
        ...
        try:
            ...
            sigmet = get_yield_curve_market_metar()
