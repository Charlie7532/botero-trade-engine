"""
Options Awareness V2 — Gamma Regime Detection Engine
=====================================================
Black-Scholes gamma exacto aplicado a opciones chains reales.

Tres estados del gamma que el engine detecta:
  1. PIN  (GEX+ & precio cerca de MP) → MMs suprimen volatilidad
  2. DRIFT (precio lejos de MP, gamma bajo) → Precio en movimiento libre
  3. SQUEEZE (GEX- & precio cruzó flip point) → MMs amplifican movimiento
...
# ═══════════════════════════════════════════════════════════════
# BLACK-SCHOLES GAMMA — ECUACIÓN EXACTA
# ═══════════════════════════════════════════════════════════════
...
def detect_opex(dt: Optional[datetime] = None) -> OpExType:
    ...
    # Determine OpEx type
    if _is_quad_witching(today):
        result.opex_type = "QUAD_WITCHING"
        result.is_opex_day = True
    elif _is_third_friday(today):
