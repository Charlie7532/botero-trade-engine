...
class RSIIntelligenceResult:
    """Result from regime-aware RSI analysis."""
    # Regime detection
...
class RSIIntelligence:
    ...
    # ── Cardwell/Brown Range Rules ─────────────────────────
    BULL_RSI_FLOOR = 40    # In bull, RSI rarely drops below 40
    BULL_RSI_CEIL = 80     # In bull, RSI reaches 80+
    BEAR_RSI_FLOOR = 20    # In bear, RSI reaches 20
    BEAR_RSI_CEIL = 60     # In bear, RSI rarely exceeds 60
    ...
    def _compute_conviction(self, result: RSIIntelligenceResult) -> float:
        ...
        score += zone_scores.get(result.rsi_zone, 0.0)
        ...
        score += div_scores.get(result.divergence_type, 0.0) * result.divergence_strength
