...
class VolumeProfileResult:
    ...
    current_vs_poc: str = "UNKNOWN"   # "ABOVE_POC", "AT_POC", "BELOW_POC"
    current_vs_va: str = "UNKNOWN"    # "ABOVE_VA", "IN_VA", "BELOW_VA"
    poc_distance_pct: float = 0.0     # % distance from current to POC
    
    # Volume Nodes
    nearest_hvn_above: float = 0.0    # Nearest resistance (HVN above)
    nearest_hvn_below: float = 0.0    # Nearest support (HVN below)
    
    # Profile shape
    shape: str = "D"                  # "P" (bullish), "D" (balanced), "b" (bearish)
    shape_skew: float = 0.0           # Positive = P, Negative = b, ~0 = D
...
class DualProfileResult:
    ...
    poc_migration: str = "NEUTRAL"    # "BULLISH", "BEARISH", "NEUTRAL"
    poc_migration_pct: float = 0.0    # % de migración del POC
    
    # Composite signal
    institutional_bias: str = "NEUTRAL"  # "ACCUMULATION", "DISTRIBUTION", "NEUTRAL"
    bias_confidence: float = 0.0         # 0-100
    
    # Actionable levels for trading
    primary_support: float = 0.0         # Best support (from both profiles)
    primary_resistance: float = 0.0      # Best resistance
    entry_zone_low: float = 0.0          # Optimal entry range
...
class VolumeProfileAnalyzer:
    ...
    P_SHAPE_SKEW_THRESHOLD = 0.15     # Skew > 0.15 = P-shape
    B_SHAPE_SKEW_THRESHOLD = -0.15    # Skew < -0.15 = b-shape
    POC_MIGRATION_THRESHOLD = 0.5     # >0.5% migration = significant
    VALUE_AREA_PCT = 0.70             # Standard 70% value area
    HVN_THRESHOLD = 1.5              # >1.5x avg volume = HVN
    LVN_THRESHOLD = 0.5              # <0.5x avg volume = LVN
