...
class MarketMETAR:
    ...
    issuer: str
    market_status: str
    vix_index_value: float
    vix_velocity_3d: float
    state_key: str
    vix_bin: str
    velocity_vector: str
    n_samples: int
    ...
    def format_cli_broadcast(self) -> str:
        ...
        return (
            ...
            f"    • VIX Index Level : {self.vix_index_value:.2f} [{self.vix_bin}]\n"
            f"    • Velocity (Δ3d)  : {self.vix_velocity_3d:+.2f} [{self.velocity_vector}]\n"
...
def get_vix_market_metar(as_of_date: Optional[str] = None) -> MarketMETAR:
    ...
    try:
        ...
        latest_bar_query = "SELECT MAX(time::date) as max_date FROM market.ohlcv_bars WHERE ticker = 'VIX' AND timeframe = '1d'"
        ...
        if as_of_date:
            sql_query = f"""
                SELECT time::date as date, close as vix
                ...
                LIMIT 30
            """
            df_vix = pd.read_sql(sql_query, engine)
        ...
        latest_row = df_vix.iloc[-1]
        t3_row = df_vix.iloc[-4]
        
        latest_date_str = str(latest_row['date'])

        vix_val = float(latest_row['vix'])
        vix_d3 = float(vix_val - float(t3_row['vix']))

        # Compute L2 Kinematic Pivot (5-day rolling volatility reversal vector)
        last5 = df_vix.tail(5)
        min5 = float(last5['vix'].min())
        max5 = float(last5['vix'].max())
        prev_val = float(df_vix.iloc[-2]['vix']) if len(df_vix) >= 2 else vix_val
        d1 = float(vix_val - prev_val)

        if vix_val >= max5 - 0.5 and d1 >= 0:
            pivot = "PANIC_SPIKE_CAPITULATION"
        elif prev_val >= max5 - 0.5 and d1 < 0:
            pivot = "VOL_CRUSH_REBOUND"
        elif vix_val <= min5 + 0.5 and d1 <= 0:
        ...
        return MarketMETAR(
            ...
            vix_index_value=vix_val,
            vix_velocity_3d=vix_d3,
            state_key=guidance.state_key,
            vix_bin=guidance.vix_bin,
