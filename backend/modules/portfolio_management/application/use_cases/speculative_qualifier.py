"""
SPECULATIVE QUALIFIER — Microstructural Walk-Forward
======================================================
Pre-trade fitness test para trades SPECULATIVE.

Diferencias con QualityQualifier:
- Usa barras HORARIAS (no diarias)
- Horizonte corto: horas/días (max_bars=10 barras horarias)
- Features microestructurales (GEX delta, flow velocity)
- Grado B suficiente (frecuencia > convicción)
- Payoff ratio más bajo (1.5-2.5x, trades rápidos)
- Menor sequence length (15 barras, no 30)
...
class SpeculativeQualifier:
    """
    Calificador Pre-Trade para SPECULATIVE.

    Karsan: "Gamma mechanics are deterministic. If GEX is negative, dealers buy on rally."
    PTJ: "Speed matters. If the setup doesn't work in 10 bars, it's wrong."

    Usa barras horarias, horizonte corto, payoff ratios moderados.
    Grade B es suficiente — frequency over conviction.
    """

    # SPECULATIVE: Grade B is sufficient
    MIN_GRADE = 'B'
    GRADE_A = 70
    GRADE_B = 55
    GRADE_C = 40
    ...
    # Speculative uses lower payoff ratios (quick trades)
    PAYOFF_RANGE = [1.5, 2.0, 2.5]
    MAX_BARS_LABEL = 10   # 10 hourly bars (~1.5 trading days)
    SEQUENCE_LENGTH = 15  # 15 hourly bars context
    ...
    def qualify(
        ...
        """
        Ejecuta fitness test SPECULATIVE: barras horarias, horizonte corto.
        Grade B o superior califica.
        """
        cache_key = f"SPECULATIVE_{ticker}_{timeframe}"
        if cache_key in self.results_cache:
            return self.results_cache[cache_key]

        logger.info(f"SPECULATIVE QUALIFIER: {ticker} @ {timeframe}")

        # Support 1h and 4h
        tf_min = 60 if timeframe == '1h' else 240

        # 1. Download hourly data
        df = self._download_data(ticker)
        if df is None or len(df) < min_bars:
            result = self._fail_result(ticker, timeframe, len(df) if df is not None else 0, f"Datos insuficientes")
        ...
        # 3. Feature engineering (hourly timeframe)
        ...
        # 4. Find optimal payoff (SPECULATIVE: lower range)
        best_payoff = 1.5
        best_ev = -999
        max_bars = self.MAX_BARS_LABEL if timeframe == '1h' else 15
        for payoff in self.PAYOFF_RANGE:
            labeler = TripleBarrierLabeler(profit_mult=payoff, loss_mult=1.0, max_bars=max_bars)
            test_df = labeler.apply_barriers(df.copy(), close_col='close')
            test_df = MetaLabeler.generate_primary_signals(test_df, momentum_lookback=10)  # 10-bar momentum for hourly
        ...
        labeler = TripleBarrierLabeler(profit_mult=best_payoff, loss_mult=1.0, max_bars=max_bars)
        ...
        if len(df) < min_bars:
            result = self._fail_result(ticker, timeframe, len(df), f"Señales insuficientes: {len(df)}")
            self.results_cache[cache_key] = result
            return result
        ...
        if not wf_results:
            result = self._fail_result(ticker, timeframe, len(df), "Walk-Forward failed")
            self.results_cache[cache_key] = result
            return result

        # 6. Compute metrics with costs
        result = self._compute_result(ticker, timeframe, df, wf_results, drift, best_payoff, SEQ)
        self.results_cache[cache_key] = result
        return result
    ...
    def _download_data(self, ticker: str) -> Optional[pd.DataFrame]:
        try:
            ...
            logger.error(f"SpeculativeQualifier download error {ticker}: {e}")
            return None
    ...
    def _compute_result(self, ticker, timeframe, df, wf_results, drift, best_payoff, seq):
        ...
        qualified = grade in ('A', 'B')  # SPECULATIVE: B is sufficient
        ...
        return QualificationResult(
            ...
            reason=f"SPECULATIVE: {grade} ({edge_score:.0f}/100) via {best_model}",
        )
