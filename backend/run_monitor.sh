...
nohup python backend/daemons/quality_daemon.py --loop 86400 > backend/quality_daemon.log 2>&1 &
echo "Quality Daemon started."

# 2. Speculative Daemon (5 minutes)
nohup python backend/daemons/speculative_daemon.py --loop 300 > backend/speculative_daemon.log 2>&1 &
echo "Speculative Daemon started."

# 3. Position Monitor (5 minutes) - Continues tracking open positions
nohup python backend/daemons/position_monitor.py --loop 300 > backend/position_monitor.log 2>&1 &
