#!/usr/bin/env python3
"""
Audit YIELD_CURVE Deep Learning & DSR Validation Script (Task 8)
================================================================
Executes:
...
5. Generates permanent documentation .agents/references/yield_curve_intelligence.md.
"""
import sys
import math
import json
...
logger = logging.getLogger("YieldCurveAudit")

REF_OUTPUT_PATH = root_dir / ".agents/references/yield_curve_intelligence.md"
...
def main():
    logger.info("⚡ Starting YIELD_CURVE Deep Learning & DSR Audit (Task 8)...")
    store = TimescaleDataStore()
    
    bars_tnx = store.load_bars("TNX", "1d")
    bars_irx = store.load_bars("IRX", "1d")
    bars_spy = store.load_bars("SPY", "1d")
    
    if bars_tnx is None or bars_irx is None or bars_spy is None:
        logger.error("❌ Missing required TNX / IRX / SPY bars in Neon Vault!")
        return

    bars_tnx = bars_tnx.sort_index()
    bars_irx = bars_irx.sort_index()
    ...
    logger.info(f"📊 Aligned YIELD_CURVE population: {len(df)} daily bars")

    # 1. Stationarity Test
    d_opt = 0.45
    fd_series = frac_diff(df['spread'], d=d_opt)
    fd_std = float(fd_series.std())

    # 2. Epistemic Uncertainty & DSR
    df['spread_d3'] = df['spread'] - df['spread'].shift(3)
    df = df.dropna()
    
    probs = []
    returns_list = []
    for idx, row in df.iterrows():
        g = yield_curve_lookup.lookup_yield_curve_guidance(row['spread'], row['spread_d3'])
        if g:
    ...
    # 3. Generate Reference Document: .agents/references/yield_curve_intelligence.md
    doc_content = f"""# Macro Yield Curve Spread Intelligence — Reference Document
...
- **Diferenciación Fraccional ($d=0.45$)**: Estacionariedad cuantitativa del spread macro de tasas (Std = {fd_std:.4f}).
...
"""
    ...
    print("✅ Task 8 (YIELD_CURVE) Deep Learning Audit Completed Successfully!")
