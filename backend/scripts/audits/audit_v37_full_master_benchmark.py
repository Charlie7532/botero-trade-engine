"""
V37.1 Master Benchmark, Zero-Assumption Audit & Regime Efficiency (2000 - 2026)
================================================================================
Audits QualityEntryGate V37.1:
  - Zero-Fallback Verification: Direct Neon PostgreSQL Data Vault reads.
  - Comprehensive Efficiency Metrics: Win Rate (%), Profit Factor, Avg Return, Max Drawdown.
  - Master Yearly Compounding & Regime Breakdown Tables.
"""
...
def audit_data_integrity(store):
    """
    Verifica que NO existan valores nulos (NaN), ni fallbacks en la base de datos de Neon.
    """
    conn = store._conn()
    try:
        ...
        macro_pivot = macro_pivot.loc[common_dates]
        
        null_counts = pivot.isnull().sum().sum() + macro_pivot.isnull().sum().sum()
        return pivot, macro_pivot, null_counts
    finally:
        store._put(conn)

def run_v37_benchmark(pivot, macro_pivot):
...
def main():
    ...
    pivot, macro_pivot, null_counts = audit_data_integrity(store)
    store.close()
    
    print("\n" + "="*115)
    print("      🔍 AUDITORÍA CERO-FALLBACKS Y BENCHMARK MAESTRO V37.1 (2000 - 2026)")
    print("="*115)
    print(f"📌 Estado de Integridad de Datos en Vault: {len(pivot)} Días Evaluados | {null_counts} Nulos en Matriz (100% Datos Reales)")
    
    df_res = run_v37_benchmark(pivot, macro_pivot)
    df_res['ret'] = df_res['equity'].pct_change().fillna(0.0)
    
    # 1. Tabla Año por Año
    print("\n" + "="*115)
    print("      📊 TABLA AÑO POR AÑO: VERSIÓN 37.1 OPTIMIZADA EN PRODUCCIÓN")
    print("="*115)
    ...
    print("\n" + "="*115)
    print(f"  ACCIONES TOTALES COMPOUNDING V37.1 : {df_res.iloc[-1]['spy_shares']:.2f} Acciones SPY (vs 100.00 iniciales)")
    print("="*115)
    
    # 2. Tabla por Régimen y Eficiencia de Operación
    print("\n" + "="*115)
    print("      🛡️ EFICIENCIA Y EFECTIVIDAD POR RÉGIMEN DE MERCADO (V37.1)")
    print("="*115)
    print(f"{'Régimen':<26s} | {'Días':<6s} | {'Ret. Acum. (%)':<16s} | {'Win Rate (%)':<14s} | {'Profit Factor':<15s} | {'Ret. Prom/Día'}")
    print("-" * 95)
