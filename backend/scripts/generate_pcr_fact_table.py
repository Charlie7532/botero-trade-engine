#!/usr/bin/env python3
"""
Generate Empirical CBOE Equity Put/Call Ratio (CBOE_PCR) Fact Store Table
=========================================================================
...
logger = logging.getLogger("GeneratePCRFactTable")
...
def calculate_pure_zigzag_pcr_stats(repo: ZigzagLegRepository, df_aligned: pd.DataFrame):
    pcr_edges = [float(x) for x in df_aligned['pcr'].quantile(PERCENTILES_7)]
    pcr_d3 = df_aligned['pcr'].diff(3)
    vel_edges = [float(x) for x in pcr_d3.dropna().quantile(PERCENTILES_7)]
    ...
    for scale in ["zz25", "zz50", "zz75"]:
        ...
        if not df_legs.empty:
            df_legs["pcr_bin"] = df_legs["pcr_at_start"].apply(lambda v: classify_bin(v, pcr_edges))
            df_legs["pcr_speed"] = "STABLE_3D"
            df_legs["state_key"] = df_legs["pcr_bin"] + "__STABLE_3D"
    ...
    for state_k, df_group in state_groups:
        ...
        pcr_t0_vals = df_group['pcr'].values
        pcr_prev_vals = df_group['pcr_prev'].values
        dates_set = set(df_group.index)

        state_doc = {
            "n": int(n_state),
            "pcr_t0_stats": {
                "min": float(np.min(pcr_t0_vals)),
                "max": float(np.max(pcr_t0_vals)),
                "mean": float(np.mean(pcr_t0_vals)),
                "std": float(np.std(pcr_t0_vals)) if n_state > 1 else 0.0
            },
            "pcr_t_minus_1_stats": {
                "min": float(np.min(pcr_prev_vals)),
                "max": float(np.max(pcr_prev_vals)),
                "mean": float(np.mean(pcr_prev_vals))
            }
        }
    ...
    return final_states, pcr_edges, vel_edges


def main():
    logger.info("Cargando historia completa de PCR y SPY de Neon Vault (market.ohlcv_bars)...")
    ...
    pcr = pivot_c['CBOE_PCR'].loc[common]
    ...
    end_date = common.max()
    n_days = len(common)
    logger.info(f"Población de entrenamiento PCR: {start_date} a {end_date} ({n_days} días hábiles / {n_days/252:.2f} años)")

    pcr_edges = [float(x) for x in pcr.quantile(PERCENTILES_7)]
    pcr_d3 = pcr.diff(3)
    vel_edges = [float(x) for x in pcr_d3.dropna().quantile(PERCENTILES_7)]

    pcr_min5 = pcr.rolling(5).min()
    pcr_max5 = pcr.rolling(5).max()
    pcr_d1 = pcr.diff(1)
    ...
    df_aligned = pd.DataFrame({
        ...
        'low': spy_l,
        'pcr': pcr,
        'pcr_prev': pcr.shift(1),
        'pcr_bin': pcr.apply(lambda v: classify_bin(v, pcr_edges)),
        'pcr_speed': pcr_d3.apply(lambda v: classify_speed(v, vel_edges)),
        'pcr_pivot': pivot_labels
    }, index=common).dropna()

    df_aligned['state_key'] = df_aligned['pcr_bin'] + "__" + df_aligned['pcr_speed'] + "__" + df_aligned['pcr_pivot']

    pure_states, pcr_edges, vel_edges = calculate_pure_zigzag_pcr_stats(repo, df_aligned)

    fact_store = {
        "_documentation": {
            "model_purpose": "CBOE Equity Put/Call Ratio (PCR) Pure Physical ZigZag Leg Expected Value Matrix",
            ...
            "state_hierarchy": {
                "L0": "PCR_Granular_Band_Percentiles_7",
                "L1": "PCR_3Day_Fast_Velocity_Percentiles_7"
            },
            "dimension_thresholds_definition": {
                "pcr_percentiles": PERCENTILES_7,
                "pcr_edges": pcr_edges,
                "pcr_speed_percentiles": PERCENTILES_7,
                "pcr_speed_edges": vel_edges,
                "pcr_labels_l0": LABELS_L0,
                "pcr_labels_l1": LABELS_L1
            },
            "field_glossary": {
                "n": "Sample size in this exact PCR stereotype",
    ...
    logger.info(f"🎉 ✅ Fact Store PURO DE ZIGZAG de PCR guardado exitosamente en {OUTPUT_PATH}")
