"""
S5 Price Breadth Triad — Derived Table Generator
=================================================
Combines price breadth data sources to produce s5_triad_derived.json.
"""
import sys
import os
import json
...
def get_s5_tickers(etf: str) -> dict:
    if etf == "SPY":
        return {"structural": "S5TH", "intermediate": "S5FI", "tactical": "S5TW"}
    return {"structural": f"S5_{etf}_TH", "intermediate": f"S5_{etf}_FI", "tactical": f"S5_{etf}_TW"}
...
print("Loading price breadth data from Vault...")
spy_tickers = get_s5_tickers("SPY")
...
for state_key, cell in cells.items():
    ...
    zone_desc = f"PTH={ZONE_MAP[th_bin]}, PFI={ZONE_MAP[fi_bin]}, PTW={ZONE_MAP[tw_bin]}, DIR={dir_desc}"
    ...
    state_entry = {
        "identity": {
            "pth_zone": ZONE_MAP[th_bin],
            "pfi_zone": ZONE_MAP[fi_bin],
            "ptw_zone": ZONE_MAP[tw_bin],
            "direction": dir_desc,
...
output_path = "backend/modules/entry_decision/domain/rules/s5_triad_derived.json"
output_data = {
    "version": "1.0",
