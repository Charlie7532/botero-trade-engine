#!/usr/bin/env python3
"""
Generate Empirical SV5_TURBULENCE Fact Store Table (Institutional Volume Turbulence)
===================================================================================
...
Usage:
    python3 -m backend.scripts.generate_sv5_turbulence_fact_table
"""
import os
import sys
import json
...
def calculate_pure_zigzag_sv5_turbulence_stats(repo: ZigzagLegRepository, df_aligned: pd.DataFrame):
    turb_edges = [float(x) for x in df_aligned['turbulence'].quantile(PERCENTILES_7)]
    turb_d3 = df_aligned['turbulence'].diff(3)
    ...
    for state_k, df_group in state_groups:
        ...
        turb_t0_vals = df_group['turbulence'].values
        turb_prev_vals = df_group['turbulence_prev'].values
    ...
    return final_states, turb_edges, vel_edges


def main():
    logger.info("Cargando historia completa de SV5_TURBULENCE y SPY de Neon Vault (market.ohlcv_bars)...")
    ...
    turb_min5 = turb.rolling(5).min()
    turb_max5 = turb.rolling(5).max()
    turb_d1 = turb.diff(1)
    ...
    for i in range(len(common)):
        v = turb.iloc[i]
        m5 = turb_min5.iloc[i]
        mx5 = turb_max5.iloc[i]
        d1 = turb_d1.iloc[i]
        pv = turb.iloc[i-1] if i > 0 else v
    ...
    df_aligned = pd.DataFrame({
        ...
        'turbulence_pivot': pivot_labels
    }, index=common).dropna()

    df_aligned['state_key'] = df_aligned['turbulence_bin'] + "__" + df_aligned['turbulence_speed'] + "__" + df_aligned['turbulence_pivot']

    pure_states, turb_edges, vel_edges = calculate_pure_zigzag_sv5_turbulence_stats(repo, df_aligned)

    fact_store = {
        "_documentation": {
            "model_purpose": "SV5 Institutional Volume Turbulence Pure Physical ZigZag Leg Expected Value Matrix",
    ...
    logger.info(f"🎉 ✅ Fact Store PURO DE ZIGZAG de SV5_TURBULENCE guardado exitosamente en {OUTPUT_PATH}")
