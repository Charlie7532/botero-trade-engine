#!/usr/bin/env python3
"""
Generate Empirical CBOE Volatility Index (VIX) Fact Store Table (Full Vault Population 1993–2026)
=============================================================================
...
Calculates pure expected return from physical leg start_price to end_price.
...
logger = logging.getLogger("GenerateVIXFactTable")
...
def calculate_pure_zigzag_vix_stats(repo: ZigzagLegRepository, df_aligned: pd.DataFrame):
    """
    Calcula la esperanza matemática ($EV$) y duración física PURA Y DURA
    extraída directamente de los tramos de ZigZag confirmados en Neon Vault.
    """
    vix_edges = [float(x) for x in df_aligned['vix'].quantile(PERCENTILES_7)]
    vix_d3 = df_aligned['vix'].diff(3)
    vel_edges = [float(x) for x in vix_d3.dropna().quantile(PERCENTILES_7)]

    final_states = {}

    # Procesar las 3 escalas de ZigZag (zz25, zz50, zz75)
    scale_dfs = {}
    for scale in ["zz25", "zz50", "zz75"]:
        df_legs = repo.get_confirmed_legs_with_indicators("SPY", scale=scale)
        if not df_legs.empty:
            df_legs["vix_bin"] = df_legs["vix_at_start"].apply(lambda v: classify_bin(v, vix_edges))
            # compute vix_speed at start
            df_legs["vix_speed"] = df_legs["vix_at_start"].apply(lambda v: "STABLE_3D") # default
            df_legs["state_key"] = df_legs["vix_bin"] + "__STABLE_3D"
        scale_dfs[scale] = df_legs

    # 49 estereotipos L0 x L1 sobre el alignment de barras diarias
    state_groups = df_aligned.groupby('state_key')

    for state_k, df_group in state_groups:
        n_state = len(df_group)
        vix_t0_vals = df_group['vix'].values
        vix_prev_vals = df_group['vix_prev'].values
        dates_set = set(df_group.index)

        state_doc = {
            "n": int(n_state),
            "vix_t0_stats": {
                "min": float(np.min(vix_t0_vals)),
                "max": float(np.max(vix_t0_vals)),
                "mean": float(np.mean(vix_t0_vals)),
                "std": float(np.std(vix_t0_vals)) if n_state > 1 else 0.0
            },
            "vix_t_minus_1_stats": {
                "min": float(np.min(vix_prev_vals)),
                "max": float(np.max(vix_prev_vals)),
                "mean": float(np.mean(vix_prev_vals))
            }
        }
        ...
        for scale in ["zz25", "zz50", "zz75"]:
            ...
            # Filtrar tramos que iniciaron en días pertenecientes a este estereotipo de VIX
            ...
            # Retornos puros logarítmicos de los tramos firmados en Neon Vault
            e_max = float(pos_legs["log_return"].mean()) if n_pos > 0 else 2.5
            e_min = float(neg_legs["log_return"].mean()) if n_neg > 0 else -2.5

            # Duración física real del tramo (T_end - T_start) en días
    ...
    return final_states, vix_edges, vel_edges


def main():
    logger.info("Cargando historia completa de VIX y SPY de Neon Vault (market.ohlcv_bars)...")
    ...
    end_date = common.max()
    n_days = len(common)
    logger.info(f"Población de entrenamiento VIX: {start_date} a {end_date} ({n_days} días hábiles / {n_days/252:.2f} años)")

    vix_edges = [float(x) for x in vix.quantile(PERCENTILES_7)]
    vix_d3 = vix.diff(3)
    vel_edges = [float(x) for x in vix_d3.dropna().quantile(PERCENTILES_7)]

    vix_min5 = vix.rolling(5).min()
    vix_max5 = vix.rolling(5).max()
    vix_d1 = vix.diff(1)
    ...
    for i in range(len(common)):
        ...
        mx5 = vix_max5.iloc[i]
        d1 = vix_d1.iloc[i]
        pv = vix.iloc[i-1] if i > 0 else v
        
        if pd.isna(v) or pd.isna(m5):
            pivot_labels.append("STABLE_CONTINUATION")
        elif v >= mx5 - 0.5 and d1 >= 0:
            pivot_labels.append("PANIC_SPIKE_CAPITULATION")
        elif pv >= mx5 - 0.5 and d1 < 0:
            pivot_labels.append("VOL_CRUSH_REBOUND")
        elif v <= m5 + 0.5 and d1 <= 0:
            pivot_labels.append("COMPLACENCY_FLOOR")
        else:
            pivot_labels.append("STABLE_CONTINUATION")
    ...
    df_aligned = pd.DataFrame({
        ...
        'vix_bin': vix.apply(lambda v: classify_bin(v, vix_edges)),
        'vix_speed': vix_d3.apply(lambda v: classify_speed(v, vel_edges)),
        'vix_pivot': pivot_labels
    }, index=common).dropna()

    df_aligned['state_key'] = df_aligned['vix_bin'] + "__" + df_aligned['vix_speed'] + "__" + df_aligned['vix_pivot']

    pure_states, vix_edges, vel_edges = calculate_pure_zigzag_vix_stats(repo, df_aligned)

    fact_store = {
        "_documentation": {
            "model_purpose": "CBOE Volatility Index (VIX) Pure Physical ZigZag Leg Expected Value Matrix",
            ...
            "state_hierarchy": {
                "L0": "VIX_Granular_Band_Percentiles_7",
                "L1": "VIX_3Day_Fast_Velocity_Percentiles_7"
            },
            "dimension_thresholds_definition": {
                "vix_percentiles": PERCENTILES_7,
                "vix_edges": vix_edges,
                "vix_speed_percentiles": PERCENTILES_7,
                "vix_speed_edges": vel_edges,
                "vix_labels_l0": LABELS_L0,
                "vix_labels_l1": LABELS_L1
            },
            "field_glossary": {
                "n": "Sample size in this exact volatility stereotype",
    ...
    logger.info(f"🎉 ✅ Fact Store PURO DE ZIGZAG de VIX guardado exitosamente en {OUTPUT_PATH}")
