...
Usage:
    python3 -m backend.scripts.generate_vvix_fact_table
"""
import os
import sys
import json
...
def calculate_pure_zigzag_vvix_stats(repo: ZigzagLegRepository, df_aligned: pd.DataFrame):
    vvix_edges = [float(x) for x in df_aligned['vvix'].quantile(PERCENTILES_7)]
    vvix_d3 = df_aligned['vvix'].diff(3)
    ...
    for scale in ["zz25", "zz50", "zz75"]:
        ...
        if not df_legs.empty:
            df_legs["vvix_bin"] = df_legs["vix_at_start"].apply(lambda v: classify_bin(v, vvix_edges)) if "vix_at_start" in df_legs.columns else "NORMAL_VVIX"
            df_legs["vvix_speed"] = "STABLE_3D"
            df_legs["state_key"] = df_legs["vvix_bin"] + "__STABLE_3D"
    ...
    for state_k, df_group in state_groups:
        ...
        vvix_t0_vals = df_group['vvix'].values
        vvix_prev_vals = df_group['vvix_prev'].values
    ...
    return final_states, vvix_edges, vel_edges


def main():
    logger.info("Cargando historia completa de VVIX y SPY de Neon Vault (market.ohlcv_bars)...")
    ...
    vvix_min5 = vvix.rolling(5).min()
    vvix_max5 = vvix.rolling(5).max()
    vvix_d1 = vvix.diff(1)
    ...
    for i in range(len(common)):
        v = vvix.iloc[i]
        m5 = vvix_min5.iloc[i]
        mx5 = vvix_max5.iloc[i]
        d1 = vvix_d1.iloc[i]
        pv = vvix.iloc[i-1] if i > 0 else v
    ...
    df_aligned = pd.DataFrame({
        ...
        'vvix_pivot': pivot_labels
    }, index=common).dropna()

    df_aligned['state_key'] = df_aligned['vvix_bin'] + "__" + df_aligned['vvix_speed'] + "__" + df_aligned['vvix_pivot']

    pure_states, vvix_edges, vel_edges = calculate_pure_zigzag_vvix_stats(repo, df_aligned)

    fact_store = {
        "_documentation": {
            "model_purpose": "CBOE Volatility of Volatility Index (VVIX) Pure Physical ZigZag Leg Expected Value Matrix",
            ...
            "field_glossary": {
                "n": "Sample size in this exact VVIX stereotype",
    ...
    logger.info(f"🎉 ✅ Fact Store PURO DE ZIGZAG de VVIX guardado exitosamente en {OUTPUT_PATH}")
