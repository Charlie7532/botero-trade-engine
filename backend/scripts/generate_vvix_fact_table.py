#!/usr/bin/env python3
"""
Generate Empirical CBOE Volatility of Volatility Index (VVIX) Fact Store Table
=============================================================================
Calculates exact empirical asymmetric percentiles for VVIX index (L0 - 7 Bins)
and VVIX 3-day Fast Kinematic Velocity (L1 - 7 Vectors) across historical
aligned market population in Neon Vault:
- EXTREME_VVIX_CRUSH_3D (Delta_3d <= P05)
- VVIX_UNWIND_3D (P05 < Delta_3d <= P15)
- DECELERATING_3D (P15 < Delta_3d <= P35)
- STABLE_3D (P35 < Delta_3d <= P65)
- RISING_3D (P65 < Delta_3d <= P85)
- VVIX_ACCUMULATION_3D (P85 < Delta_3d <= P95)
- EXTREME_VVIX_SPIKE_3D (Delta_3d > P95)
...
Usage:
    backend/.venv/bin/python3 -m backend.scripts.generate_vvix_fact_table
"""
import os
import sys
import json
...
logger = logging.getLogger("GenerateVVIXFactTable")

OUTPUT_PATH = root_dir / "backend/modules/entry_decision/domain/rules/vvix_fact_store.json"
...
def calculate_3d_vvix_stats(df_aligned: pd.DataFrame):
    prices_c = df_aligned['close'].values
    prices_h = df_aligned['high'].values
    prices_l = df_aligned['low'].values
    vvix_vals = df_aligned['vvix'].values
    vvix_prev = df_aligned['vvix_prev'].values
    ...
    for state_k, idx_list in state_indices.items():
        ...
        vvix_t0_vals = vvix_vals[idx_list]
        vvix_prev_vals = vvix_prev[idx_list]

        state_doc = {
            "n": int(n_state),
            "vvix_t0_stats": {
                "min": float(np.min(vvix_t0_vals)),
                "max": float(np.max(vvix_t0_vals)),
                "mean": float(np.mean(vvix_t0_vals)),
                "std": float(np.std(vvix_t0_vals)) if n_state > 1 else 0.0
            },
            "vvix_t_minus_1_stats": {
                "min": float(np.min(vvix_prev_vals)),
                "max": float(np.max(vvix_prev_vals)),
                "mean": float(np.mean(vvix_prev_vals))
            }
        }
        ...
        for target_pct in ZIGZAG_LEVELS:
            ...
            for i in idx_list:
                p0 = prices_c[i]
                local_vvix = vvix_vals[i]
                # 25 bps friction in panic regime (VVIX > 120), 10 bps standard
                friction = 0.0025 if local_vvix > 120.0 else 0.0010
...
def main():
    logger.info("Cargando historia de VVIX y SPY desde Neon Vault (market.ohlcv_bars)...")
    ...
    end_date = common.max()
    n_days = len(common)
    logger.info(f"Población de entrenamiento VVIX: {start_date} a {end_date} ({n_days} días hábiles / {n_days/252:.2f} años)")

    # Compute 7-scale percentiles for L0 (VVIX Bins) and L1 (3-Day Fast Velocity Vectors)
    vvix_edges = [float(x) for x in vvix.quantile(PERCENTILES_7)]
    vvix_d3 = vvix.diff(3)
    vel_edges = [float(x) for x in vvix_d3.dropna().quantile(PERCENTILES_7)]

    logger.info(f"Cortes VVIX Level (L0): {vvix_edges}")
    logger.info(f"Cortes VVIX Velocity 3-Day (L1): {vel_edges}")
    ...
    df_aligned = pd.DataFrame({
        ...
        'low': spy_l,
        'vvix': vvix,
        'vvix_prev': vvix.shift(1),
        'vvix_bin': vvix.apply(lambda v: classify_bin(v, vvix_edges)),
        'vvix_speed': vvix_d3.apply(lambda v: classify_speed(v, vel_edges))
    }).dropna()

    df_aligned['state_key'] = df_aligned['vvix_bin'] + "__" + df_aligned['vvix_speed']

    # Compute pure VVIX 3-day volatility statistics using Intraday High/Low FTT
    pure_states = calculate_3d_vvix_stats(df_aligned)
    ...
    for l0 in LABELS_L0:
        for l1 in LABELS_L1:
            ...
            if k not in pure_states:
                ...
                pure_states[k] = {
                    "n": 0,
                    "vvix_t0_stats": {"min": 0.0, "max": 0.0, "mean": 0.0, "std": 0.0},
                    "vvix_t_minus_1_stats": {"min": 0.0, "max": 0.0, "mean": 0.0},
    ...
    fact_store = {
        "_documentation": {
            "model_purpose": "CBOE Volatility of Volatility Index (VVIX) 3-Day Fast Kinematic Velocity Matrix (Full Vault Population)",
            "return_formula": "R_net = (P_ftt / P_t) - 1.0 - friction (25bps in panic VVIX > 120, 10bps standard)",
            ...
            "state_hierarchy": {
                "L0": "VVIX_Granular_Band_Percentiles_7",
                "L1": "VVIX_3Day_Fast_Velocity_Percentiles_7"
            },
            "dimension_thresholds_definition": {
                "vvix_percentiles": PERCENTILES_7,
                "vvix_edges": vvix_edges,
                "vvix_speed_percentiles": PERCENTILES_7,
                "vvix_speed_edges": vel_edges,
                "vvix_labels_l0": LABELS_L0,
                "vvix_labels_l1": LABELS_L1
            },
            "field_glossary": {
    ...
    logger.info(f"Fact Store VVIX 3-Day Fast Velocity guardado exitosamente en {OUTPUT_PATH}")
