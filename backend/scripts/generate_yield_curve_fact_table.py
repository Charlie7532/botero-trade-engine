#!/usr/bin/env python3
"""
Generate Empirical Yield Curve (10Y-2Y Spread) Fact Store Table
=============================================================================
...
Usage:
    python3 -m backend.scripts.generate_yield_curve_fact_table
"""
import os
import sys
import json
...
def calculate_pure_zigzag_yield_curve_stats(repo: ZigzagLegRepository, df_aligned: pd.DataFrame):
    yc_edges = [float(x) for x in df_aligned['yield_curve'].quantile(PERCENTILES_7)]
    yc_d3 = df_aligned['yield_curve'].diff(3)
    vel_edges = [float(x) for x in yc_d3.dropna().quantile(PERCENTILES_7)]
    ...
    for scale in ["zz25", "zz50", "zz75"]:
        ...
        if not df_legs.empty:
            df_legs["yield_curve_bin"] = df_legs["vix_at_start"].apply(lambda v: classify_bin(v, yc_edges)) if "vix_at_start" in df_legs.columns else "NORMAL_CURVE"
            df_legs["yield_curve_speed"] = "STABLE_3D"
            df_legs["state_key"] = df_legs["yield_curve_bin"] + "__STABLE_3D"
    ...
    for state_k, df_group in state_groups:
        ...
        yc_t0_vals = df_group['yield_curve'].values
        yc_prev_vals = df_group['yield_curve_prev'].values
        dates_set = set(df_group.index)

        state_doc = {
            "n": int(n_state),
            "yield_curve_t0_stats": {
                "min": float(np.min(yc_t0_vals)),
                "max": float(np.max(yc_t0_vals)),
                "mean": float(np.mean(yc_t0_vals)),
                "std": float(np.std(yc_t0_vals)) if n_state > 1 else 0.0
            },
            "yield_curve_t_minus_1_stats": {
                "min": float(np.min(yc_prev_vals)),
                "max": float(np.max(yc_prev_vals)),
                "mean": float(np.mean(yc_prev_vals))
            }
        }
    ...
    return final_states, yc_edges, vel_edges


def main():
    logger.info("Cargando historia completa de TNX / Yield Curve y SPY de Neon Vault (market.ohlcv_bars)...")
    ...
    end_date = common.max()
    n_days = len(common)
    logger.info(f"Población de entrenamiento Yield Curve: {start_date} a {end_date} ({n_days} días hábiles / {n_days/252:.2f} años)")

    yc_edges = [float(x) for x in tnx.quantile(PERCENTILES_7)]
    yc_d3 = tnx.diff(3)
    vel_edges = [float(x) for x in yc_d3.dropna().quantile(PERCENTILES_7)]

    yc_min5 = tnx.rolling(5).min()
    yc_max5 = tnx.rolling(5).max()
    yc_d1 = tnx.diff(1)
    ...
    df_aligned = pd.DataFrame({
        ...
        'low': spy_l,
        'yield_curve': tnx,
        'yield_curve_prev': tnx.shift(1),
        'yield_curve_bin': tnx.apply(lambda v: classify_bin(v, yc_edges)),
        'yield_curve_speed': yc_d3.apply(lambda v: classify_speed(v, vel_edges)),
        'yield_curve_pivot': pivot_labels
    }, index=common).dropna()

    df_aligned['state_key'] = df_aligned['yield_curve_bin'] + "__" + df_aligned['yield_curve_speed'] + "__" + df_aligned['yield_curve_pivot']

    pure_states, yc_edges, vel_edges = calculate_pure_zigzag_yield_curve_stats(repo, df_aligned)

    fact_store = {
        "_documentation": {
            "model_purpose": "Yield Curve (10Y Yield) Pure Physical ZigZag Leg Expected Value Matrix",
            ...
            "state_hierarchy": {
                "L0": "Yield_Curve_Granular_Band_Percentiles_7",
                "L1": "Yield_Curve_3Day_Fast_Velocity_Percentiles_7"
            },
            "dimension_thresholds_definition": {
                "yield_curve_percentiles": PERCENTILES_7,
                "yield_curve_edges": yc_edges,
                "yield_curve_speed_percentiles": PERCENTILES_7,
                "yield_curve_speed_edges": vel_edges,
                "yield_curve_labels_l0": LABELS_L0,
                "yield_curve_labels_l1": LABELS_L1
            },
            "field_glossary": {
    ...
    logger.info(f"🎉 ✅ Fact Store PURO DE ZIGZAG de Yield Curve guardado exitosamente en {OUTPUT_PATH}")
